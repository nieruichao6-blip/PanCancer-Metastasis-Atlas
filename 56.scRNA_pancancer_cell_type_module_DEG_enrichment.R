...
# data read ---------------------------------------------------------------
## DEG read
all_cell_type_DEG <- read_csv(str_glue("{input_dir}/all_DEG.csv"))

## all gene read
all_gene_data <- readRDS(all_gene_dir)
...
for(CM_type in c("CM1","CM2","CM3","CM4","CM5","CM6","CM7","CM8","CM9")){
  ...
  current_DEG <- all_cell_type_DEG %>% 
    filter(group %in% all_CM[[CM_type]]) %>% 
    filter(logfoldchanges >= log2(1.5) & pct_nz_group >= 0.5) %>% 
    group_by(group) %>% 
    slice_max(order_by = logfoldchanges, n = 10) %>% 
    # arrange(desc(logfoldchanges), .by_group = T) %>% 
    # slice_head(n = 15) %>% 
    ungroup()
  
  ## hub gene
  hub_DEG <- unique(current_DEG$names)
