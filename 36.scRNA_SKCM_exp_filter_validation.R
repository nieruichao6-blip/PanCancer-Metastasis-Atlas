...
input_dir <- " "
output_dir <- " "
nor_method <- "CPM_origin"


# data read ---------------------------------------------------------------
scRNA_tumor_origin <- read_h5(file = str_glue("{input_dir}/scRNA_Melanoma_tumor.h5"),
                              assay.name = 'RNA', 
...
for(sample_id in sample_list){
  ...
  ## data save
  saveRDS(exp_matrix,str_glue("{output_dir}/01.tumor_cell_exp_filter/{sample_id}_exp.RData"))
