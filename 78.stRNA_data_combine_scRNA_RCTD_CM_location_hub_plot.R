...
for(tumor in tumor_list){
  ...
  for(file in file_name){
    ...
    for(CM_type in names(all_CM_list)){
      ...
      SpatialFeaturePlot(stRNA_data,features = CM_type,alpha = c(1, 1),pt.size.factor = 4.5,stroke = NA,min.cutoff = 0,max.cutoff = 0.5) + NoLegend()
      function_plot(filename_prefix = str_glue("{output_file}/{file}_{CM_type}"), width = 200, height = 200)
